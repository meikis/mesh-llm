use crate::{
    ClientBuilder, InviteToken, MeshApiError, MeshClient, MeshNode, MeshNodeBuilder, OwnerKeypair,
};
...
impl PublicMesh {
    ...
    pub fn node_builder(
        &self,
        owner_keypair: OwnerKeypair,
    ) -> Result<MeshNodeBuilder, MeshApiError> {
        MeshNodeBuilder::from_public_mesh(owner_keypair, self)
    }
}
...
pub async fn create_auto_node(
    ...
    let mesh = select_public_mesh(query).await?;
    let node = mesh.node_builder(owner_keypair)?.build()?;
    Ok(AutoNodeResult {
        node,
        selected_mesh: mesh,
    })
...
mod tests {
    ...
    fn public_mesh_can_build_node_builder() {
        ...
        let builder = mesh.node_builder(owner_keypair);
