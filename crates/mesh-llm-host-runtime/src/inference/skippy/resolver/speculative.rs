...
pub(super) fn resolve_speculative_config(
    ...
    if mode == "auto" && strategy == "auto" && package_ngram_window.is_some() {
    ...
    let ngram_min = super::support::pick_value(
        ...
        package_ngram_window.map_or(0, |window| window.0),
    );
    let ngram_max = super::support::pick_value(
        ...
        package_ngram_window.map_or(0, |window| window.1),
    );
    ...
    if native_mtp_enabled && draft_model_path.is_some() {
        ...
        if strategy == "mtp" {
            bail!("skippy speculative ngram mode cannot also force native MTP");
        }
