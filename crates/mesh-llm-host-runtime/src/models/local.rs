...
// ── Speculative prefill auto-detect ──────────────────────────────────

/// Maximum file size for an auto-detected speculative prefill model (1 GB).
const SPEC_PREFILL_MAX_BYTES: u64 = 1_000_000_000;

/// Extract a model family prefix from a model name or ref.
///
/// Examples:
///   "unsloth/Qwen3-8B-GGUF:Q4_K_M"     → "qwen3"
///   "Qwen3-0.6B-Q4_K_M.gguf"           → "qwen3"
///   "gemma-4-E4B-it-Q4_K_M.gguf"       → "gemma"
///   "Llama-4-Scout-17B-Q4_K_M.gguf"    → "llama"
///   "MiniMax-M2.5-Q4_K_M.gguf"         → "minimax"
///   "Qwen3.5-9B-Q4_K_M.gguf"           → "qwen3"
fn extract_model_family(name: &str) -> Option<String> {
    // Strip repo prefix (e.g. "unsloth/") and quant selector (":Q4_K_M")
    let bare = name
        .rsplit('/')
        .next()
        .unwrap_or(name)
    ...
    let bare = bare.strip_suffix(".gguf").unwrap_or(bare);

    // Take the first component before any size indicator.
    // Family name: alpha chars (possibly with digits embedded in the family
    // name like "Qwen3") up to the first hyphen followed by a pure size
    // token (digit, "GGUF", etc).
    let lower = bare.to_lowercase();

    // Take the first hyphen-delimited segment as the family name.
    // Most model families are a single word: Qwen3, Llama, gemma, MiniMax, Phi.
    let family = lower.split('-').next().unwrap_or(&lower);
    if family.is_empty() {
        return None;
    }

    // Strip trailing dot-version (e.g. "qwen3.5" → "qwen3")
    // but keep trailing digits that are part of the family name (e.g. "qwen3" stays "qwen3")
    if let Some(dot_pos) = family.rfind('.') {
        let after_dot = &family[dot_pos + 1..];
        if !after_dot.is_empty() && after_dot.chars().all(|c| c.is_ascii_digit()) {
            return Some(family[..dot_pos].to_string());
        }
    }
    Some(family.to_string())
}

/// Scan the local HF cache for a small GGUF model from the same family as
/// `target_model_ref`, suitable for speculative prefill (DraftRunner).
///
/// Returns the path to the smallest same-family GGUF under [`SPEC_PREFILL_MAX_BYTES`],
/// excluding the target model itself.
pub fn find_speculative_prefill_model(target_model_ref: &str) -> Option<PathBuf> {
    let target_family = extract_model_family(target_model_ref)?;
    ...
    let candidates = scan_hf_cache_fast(&cache_root);
    ...
    for path in candidates {
        ...
        // Skip layer package files and fragments
        if file_name.starts_with("layer-")
            || file_name.starts_with("shared-")
            || file_name == "metadata.gguf"
            || file_name == "embeddings.gguf"
            || file_name == "output.gguf"
        {
            continue;
        }
        ...
        // Skip if this is the same file as the target
        if let Some(target_path) = remembered_model_ref_path(target_model_ref) {
            if path == target_path {
                continue;
            }
        }
        ...
        match &best {
            Some((_, best_size)) if file_size >= *best_size => {}
            _ => {
                best = Some((path, file_size));
            }
        }
    }
...
#[cfg(test)]
mod spec_prefill_tests {
    use super::*;

    #[test]
    fn family_extraction() {
        assert_eq!(
            extract_model_family("unsloth/Qwen3-8B-GGUF:Q4_K_M"),
            Some("qwen3".to_string())
        );
        assert_eq!(
            extract_model_family("Qwen3-0.6B-Q4_K_M.gguf"),
            Some("qwen3".to_string())
        );
        assert_eq!(
            extract_model_family("gemma-4-E4B-it-Q4_K_M.gguf"),
            Some("gemma".to_string())
        );
        assert_eq!(
            extract_model_family("Llama-4-Scout-17B-Q4_K_M.gguf"),
            Some("llama".to_string())
        );
        assert_eq!(
            extract_model_family("MiniMax-M2.5-Q4_K_M.gguf"),
            Some("minimax".to_string())
        );
        assert_eq!(
            extract_model_family("Qwen3.5-9B-GGUF:Q4_K_M"),
            Some("qwen3".to_string())
        );
    }
}
