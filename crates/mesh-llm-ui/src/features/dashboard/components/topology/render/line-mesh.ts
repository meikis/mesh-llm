import type { ColorTuple } from '@/features/dashboard/components/topology/types'
...
export function buildLineMesh({ positions, colors, lineWidthPx, devicePixelRatio }: BuildLineMeshInput): LineMesh {
  ...
  for (let segmentIndex = 0; segmentIndex < segments.length;) {
    ...
    for (let chainSegmentIndex = chainStart; chainSegmentIndex <= chainEnd; chainSegmentIndex += 1) {
      ...
      pushQuadVertex(meshPositions, meshColors, meshLineCoords, endLeftX, endLeftY, segment.endColor, endAlong, -1)
      pushQuadVertex(meshPositions, meshColors, meshLineCoords, endLeftX, endLeftY, segment.endColor, endAlong, -1)
      pushQuadVertex(
        meshPositions,
        meshColors,
      ...
      pushQuadVertex(meshPositions, meshColors, meshLineCoords, endRightX, endRightY, segment.endColor, endAlong, 1)
