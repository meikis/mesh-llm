...
pub use crate::gguf_kv_cache::{GgufKvCacheQuant, GgufKvCacheType};
...
pub fn scan_gguf_compact_meta(path: &Path) -> Option<GgufCompactMeta> {
    ...
    for _ in 0..n_kv {
        ...
        if key == "general.architecture" {
            ...
            if let Ok(Some(v)) = read_gguf_value_as_u32(&mut f, vtype) {
                meta.rope_dimension_count = v;
            }
        } else if key.ends_with(".attention.indexer.key_length") {
            if let Ok(Some(v)) = read_gguf_value_as_u32(&mut f, vtype) {
                meta.indexer_key_length = v;
            }
...
mod tests {
    ...
    #[test]
    fn scan_gguf_compact_meta_preserves_glm_dsa_cache_shape() {
        let mut bytes = Vec::new();
        bytes.extend_from_slice(b"GGUF");
        bytes.extend_from_slice(&2u32.to_le_bytes());
        bytes.extend_from_slice(&0i64.to_le_bytes());
        ...
        push_gguf_string(&mut bytes, "glm-dsa");
        push_u32_kv(&mut bytes, "glm-dsa.block_count", 79);
        push_u32_kv(&mut bytes, "glm-dsa.attention.head_count", 64);
        push_u32_kv(&mut bytes, "glm-dsa.attention.head_count_kv", 64);
        push_u32_kv(&mut bytes, "glm-dsa.attention.key_length", 576);
        push_u32_kv(&mut bytes, "glm-dsa.attention.value_length", 256);
        push_u32_kv(&mut bytes, "glm-dsa.attention.kv_lora_rank", 512);
        push_u32_kv(&mut bytes, "glm-dsa.rope.dimension_count", 64);
        push_u32_kv(&mut bytes, "glm-dsa.attention.indexer.key_length", 128);

        let path = write_bytes("model-artifact-gguf-glm-dsa-cache", &bytes);
        let meta = scan_gguf_compact_meta(&path).expect("should parse GGUF");
        assert_eq!(meta.architecture, "glm-dsa");
        assert_eq!(meta.kv_lora_rank, 512);
        assert_eq!(meta.rope_dimension_count, 64);
        assert_eq!(meta.indexer_key_length, 128);
        assert_eq!(
