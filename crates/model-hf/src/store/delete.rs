...
pub trait DeleteModelCatalog {
    fn local_stem_for_identifier(&self, identifier: &str) -> Option<String>;
}

pub struct NoDeleteCatalog;

impl DeleteModelCatalog for NoDeleteCatalog {
    fn local_stem_for_identifier(&self, _identifier: &str) -> Option<String> {
        None
    }
}
...
async fn parse_delete_model_ref(
    ...
    if let Some(stem) = catalog.local_stem_for_identifier(input)
        && find_model_path(&stem).exists()
    {
        return Ok(DeleteModelRef::LocalStem(stem));
    }
    ...
    if let Some((repo, revision, file)) = parse_huggingface_file_ref(input) {
        return Ok(DeleteModelRef::HuggingFace {
            repo,
            revision,
            file,
        });
    ...
    bail!(
        "Expected a model stem or Hugging Face ref like org/repo, org/repo@rev:QUANT, org/repo/file.gguf, or org/repo/file-stem for split GGUFs."
    )
}

fn parse_huggingface_file_ref(input: &str) -> Option<(String, Option<String>, String)> {
...
pub async fn resolve_huggingface_file_from_sibling_entries(
    repo: &str,
    revision: Option<&str>,
    file: &str,
    ...
    let has_mlx_weights = siblings
        .iter()
        .any(|entry| entry.path == "model.safetensors" || is_split_mlx_first_shard(&entry.path));
    ...
    let selector = (!file.is_empty()).then_some(file);
    let selected = select_primary_artifact_file(selector, &siblings).with_context(|| {
        let revision = revision.unwrap_or("main");
        if file.is_empty() {
            format!("No model files found in {repo}@{revision}")
        } else {
            format!("No model file matching stem '{file}' in {repo}@{revision}")
        }
    })?;
    Ok(selected.path)
}
...
pub async fn resolve_model_identifier(identifier: &str) -> Result<Vec<PathBuf>> {
    resolve_model_identifier_with_catalog(identifier, &NoDeleteCatalog).await
}

pub async fn resolve_model_identifier_with_catalog(
    identifier: &str,
    catalog: &impl DeleteModelCatalog,
) -> Result<Vec<PathBuf>> {
    match parse_delete_model_ref(identifier, catalog).await? {
        DeleteModelRef::LocalStem(stem) => {
            let path = find_model_path(&stem);
            if !path.exists() {
...
pub async fn delete_model_by_identifier(identifier: &str) -> Result<DeleteResult> {
    delete_model_by_identifier_with_catalog(identifier, &NoDeleteCatalog).await
}

pub async fn delete_model_by_identifier_with_catalog(
    identifier: &str,
    catalog: &impl DeleteModelCatalog,
) -> Result<DeleteResult> {
