...
const GLM4_MOE_LITE_ROPE_THETA: u64 = 1_000_000;
const GLM4_MOE_LITE_ROTARY_DIM: usize = 64;
...
fn resolve_rotary_config(manifest: &SpdHeadManifest, head_dim: usize) -> Result<(usize, f32)> {
    match (manifest.topology.rotary_dim, manifest.topology.rope_theta) {
        ...
        (None, None) if can_use_glm4_moe_lite_rotary_defaults(manifest) => {
            validate_rotary_dim(GLM4_MOE_LITE_ROTARY_DIM, head_dim)?;
            Ok((GLM4_MOE_LITE_ROTARY_DIM, GLM4_MOE_LITE_ROPE_THETA as f32))
        }
        (None, None) => bail!(
...
fn can_use_glm4_moe_lite_rotary_defaults(manifest: &SpdHeadManifest) -> bool {
    let base_model_path = manifest.source.base_model_path.to_ascii_lowercase();
    let model_type = manifest
        ...
        .unwrap_or_default()
        .to_ascii_lowercase();
    model_type == "glm4_moe_lite" || base_model_path.contains("glm-4.7")
}
...
mod tests {
    use super::*;
    use crate::spd::{
        SPD_HEAD_MANIFEST_SCHEMA, SpdHeadCheckpoint, SpdHeadSource, SpdHeadTopology,
        TORCH_SPD_HEAD_FORMAT_V10,
    };

    #[test]
    fn glm4_moe_lite_manifest_uses_glm_rotary_defaults() {
        let manifest = test_manifest("zai-org/GLM-4.7-Flash", Some("glm4_moe_lite"));

        let (rotary_dim, rope_theta) = resolve_rotary_config(&manifest, 128).unwrap();

        assert_eq!(rotary_dim, GLM4_MOE_LITE_ROTARY_DIM);
        assert_eq!(rope_theta, GLM4_MOE_LITE_ROPE_THETA as f32);
    }
    ...
    fn test_manifest(base_model_path: &str, model_type: Option<&str>) -> SpdHeadManifest {
        SpdHeadManifest {
            schema: SPD_HEAD_MANIFEST_SCHEMA.to_string(),
            checkpoint: SpdHeadCheckpoint {
                path: "head.pt".to_string(),
            ...
            source: SpdHeadSource {
                ...
                model_type: model_type.map(ToString::to_string),
            ...
            topology: SpdHeadTopology {
                ...
                head_kind: None,
                num_stages: 3,
                stage_layer_boundaries: Some(vec![15, 31, 47]),
                num_spec_layers: 1,
                max_taps: None,
                tap_feature_size: None,
                trained_with_use_deepest: true,
                shallow_hidden_layer_indices: vec![vec![0, 15, 31, 47]],
