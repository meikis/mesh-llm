...
use skippy_runtime::package::{PackageStageRequest, select_layer_package_parts};
use skippy_runtime::spd::{
    ...
    read_gguf_output_norm_weight, required_spd_hf_indices, required_spd_hf_indices_for_topology,
    sliding_spd_row_positions, spd_fixture_cur_in_row_count, spd_fixture_row_hf_indices,
    spd_hf_indices_for_stage_id,
};
...
impl SpdReplayProposalSource {
    fn open(args: SpdReplayOpenArgs<'_>) -> Result<Self> {
        ...
        let fixture_file = args
            .fixture_path
            .map(|path| SpdSafetensorsFile::open(path).context("open SPD parity fixture"))
            .transpose()?;
        let hidden_size =
        ...
        let row_metadata = spd_replay_row_metadata(&manifest, fixture_file.as_ref())?;
        let row_count = row_metadata.row_stage_ids.len();
        ...
        let final_norm_weight = read_spd_final_norm_weight_for_runtime(
            fixture_file.as_ref(),
            &model_source,
            args.config,
            hidden_size,
        ...
        Ok(Self {
            ...
            top_k: args.top_k,
            row_count,
            row_stage_ids: row_metadata.row_stage_ids,
            row_hf_indices: row_metadata.row_hf_indices,
            hidden_size,
            final_norm_weight,
...
pub(super) fn open_spd_replay_source(
    ...
    match (args.manifest_path, args.fixture_path) {
        ...
        (None, Some(_)) => bail!("--openai-spd-fixture requires --openai-spd-manifest"),
    }
}
...
fn read_spd_final_norm_weight_for_runtime(
    fixture: Option<&SpdSafetensorsFile>,
    model_source: &SpdReplayModelSource,
    config: &StageConfig,
    hidden_size: usize,
) -> Result<Vec<f32>> {
    if let Some(fixture) = fixture {
        return read_spd_final_norm_weight(fixture, hidden_size);
    }
    match model_source {
        SpdReplayModelSource::Gguf(model_path) => {
            read_gguf_output_norm_weight(model_path, hidden_size).with_context(|| {
                format!("read SPD final norm weight from {}", model_path.display())
            })
        }
        SpdReplayModelSource::LayerPackage(package_ref) => {
            read_spd_final_norm_weight_from_layer_package(package_ref, config, hidden_size)
        }
    }
}

fn read_spd_final_norm_weight_from_layer_package(
    package_ref: &str,
    config: &StageConfig,
    hidden_size: usize,
) -> Result<Vec<f32>> {
    let selected = select_layer_package_parts(&PackageStageRequest {
        ...
        stage_id: "spd-final-norm".to_string(),
        layer_start: config.layer_end.saturating_sub(1),
        layer_end: config.layer_end,
        include_embeddings: false,
        include_output: true,
    })
    .context("select layer package output part for SPD final norm")?;
    let output_part = selected
        .selected_parts
        .iter()
        .zip(selected.absolute_paths.iter())
        .find_map(|(part, path)| (part.role == "output" && path.is_file()).then_some(path))
        .context("layer package output selection did not contain a GGUF part")?;
    read_gguf_output_norm_weight(output_part, hidden_size).with_context(|| {
        format!(
            "read SPD final norm weight from layer package output {}",
            output_part.display()
        )
    })
}

fn spd_replay_row_metadata(
    manifest: &SpdHeadManifest,
    fixture: Option<&SpdSafetensorsFile>,
) -> Result<SpdReplayRowMetadata> {
    if let Some(fixture) = fixture {
        let row_count = spd_fixture_cur_in_row_count(fixture, manifest.topology.hidden_size)?;
        return Ok(SpdReplayRowMetadata {
            row_stage_ids: read_spd_row_stage_ids(fixture, row_count)?,
            row_hf_indices: spd_fixture_row_hf_indices(fixture, row_count)?,
        });
    }
    let stage_count =
        usize::try_from(manifest.topology.num_stages).context("SPD num_stages exceeds usize")?;
    if stage_count == 0 {
        bail!("SPD manifest num_stages must be greater than zero");
    }
    let row_count = stage_count
        .checked_add(1)
        .context("SPD replay row count overflow")?;
    let mut row_stage_ids = Vec::with_capacity(row_count);
    let mut row_hf_indices = Vec::with_capacity(row_count);
    for stage_id in (0..=stage_count).rev() {
        let stage_id_u32 = u32::try_from(stage_id).context("SPD stage id exceeds u32")?;
        row_stage_ids.push(i64::try_from(stage_id).context("SPD stage id exceeds i64")?);
        row_hf_indices.push(spd_hf_indices_for_stage_id(
            &manifest.topology,
            stage_id_u32,
        )?);
    }
    Ok(SpdReplayRowMetadata {
        row_stage_ids,
        row_hf_indices,
    })
}
