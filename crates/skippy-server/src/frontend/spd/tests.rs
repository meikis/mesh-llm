...
#[test]
fn manifest_rows_supply_runtime_metadata_without_fixture() {
    let manifest = SpdHeadManifest {
        schema: skippy_runtime::spd::SPD_HEAD_MANIFEST_SCHEMA.to_string(),
        checkpoint: skippy_runtime::spd::SpdHeadCheckpoint {
            path: "speculation_head_final.pt".to_string(),
            sha256: "sha256".to_string(),
            bytes: 1,
        },
        serving_checkpoint: None,
        source: skippy_runtime::spd::SpdHeadSource {
            format: skippy_runtime::spd::TORCH_SPD_HEAD_FORMAT_V10.to_string(),
            reference_repo: None,
            base_model_path: "zai-org/GLM-4.7-Flash".to_string(),
            model_type: Some("glm4_moe_lite".to_string()),
        ...
        topology: skippy_runtime::spd::SpdHeadTopology {
            ...
            shallow_hidden_layer_indices: vec![vec![0, 15, 31, 47], vec![0, 15, 31], vec![0, 15]],
    ...
    let rows = spd_replay_row_metadata(&manifest, None).unwrap();

    assert_eq!(rows.row_stage_ids, vec![3, 2, 1, 0]);
    assert_eq!(
        rows.row_hf_indices,
        vec![vec![0, 15, 31, 47], vec![0, 15, 31], vec![0, 15], vec![0]]
    );
}
